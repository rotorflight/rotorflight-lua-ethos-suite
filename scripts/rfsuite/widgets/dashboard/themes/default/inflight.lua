...
function preflight.paint(widget)
    ...
    rfsuite.widgets.dashboard.utils.telemetryBox(0, 2, 200, 100, 2, "VOLTAGE", value, "V", false)

    -- current
    local value = telemetry.getSensorSource("current"):value()
    rfsuite.widgets.dashboard.utils.telemetryBox(202, 2, 200, 100, 1, "CURRENT", value, "A", false)

    -- current
    local value = telemetry.getSensorSource("rpm"):value()
    value = value and math.floor(value)
    rfsuite.widgets.dashboard.utils.telemetryBox(404, 2, 200, 100, 2, "RPM", value, "RPM", false)

    -- STATUS
    rfsuite.widgets.dashboard.utils.telemetryBox(0, 104, 604, 100, 1, "", "IN-FLIGHT", "", false)
