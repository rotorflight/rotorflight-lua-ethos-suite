...
function mspSendRequest(cmd, payload)
    ...
    if #(mspTxBuf) ~= 0 or not cmd then
        rfsuite.utils.log(
            "Existing mspTxBuf is still being sent, failed send of cmd: " ..
                tostring(cmd))
        return nil
    end
    mspTxBuf[1] = #(payload)
...
local function mspReceivedReply(payload)
    ...
    if idx > rfsuite.bg.msp.protocol.maxRxBufferSize then
        rfsuite.utils.log(
            "  mspReceivedReply:  payload continues into next frame.")
    ...
    if mspRxCRC ~= payload[idx] and version == 0 then
        if rfsuite.app.Page ~= nil then
            if rfsuite.app.Page.mspChecksum then
                rfsuite.app.Page.mspChecksum(payload)
            end
        end
        rfsuite.utils.log(
            "  mspReceivedReply:  payload checksum incorrect, message failed!")
        rfsuite.utils.log("        Calculated mspRxCRC:  0x" ..
                              string.format("%X", mspRxCRC))
        rfsuite.utils.log("        CRC from payload:         0x" ..
                              string.format("%X", payload[idx]))
        return nil
    end
...
function mspClearTxBuf() mspTxBuf = {} end
