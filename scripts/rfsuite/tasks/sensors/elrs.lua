...
local function setTelemetryValue(uid, subid, instance, value, unit, dec, name,
                                 min, max)

    if sensors['uid'][uid] == nil then
        sensors['uid'][uid] = system.getSource({
...
local function decU16(data, pos) return (data[pos] << 8) | data[pos + 1],
                                        pos + 2 end
...
local function decCells(data, pos)
    ...
    for i = 1, cnt do
        ...
        setTelemetryValue(0x102F, 0, 0, vol, UNIT_CELLS, 2, "Cell Voltages", 0,
                          455) -- Cels
    end
    return nil, pos
end
...
local function decControl(data, pos)
    ...
    setTelemetryValue(0x1031, 0, 0, p, UNIT_DEGREE, 2, "Pitch Control", -4500,
                      4500) -- CPtc
    setTelemetryValue(0x1032, 0, 0, r, UNIT_DEGREE, 2, "Roll Control", -4500,
                      4500) -- CRol
    setTelemetryValue(0x1033, 0, 0, 3 * y, UNIT_DEGREE, 2, "Yaw Control", -9000,
                      9000) -- CYaw
    setTelemetryValue(0x1034, 0, 0, c, UNIT_DEGREE, 2, "Coll Control", -4500,
                      4500) -- CCol
    return nil, pos
end
...
local function decAttitude(data, pos)
    ...
    setTelemetryValue(0x1101, 0, 0, p, UNIT_DEGREE, 1, "Pitch Attitude", -1800,
                      3600) -- Ptch
    setTelemetryValue(0x1102, 0, 0, r, UNIT_DEGREE, 1, "Roll Attitude", -1800,
                      3600) -- Roll
    setTelemetryValue(0x1103, 0, 0, y, UNIT_DEGREE, 1, "Yaw Attitude", -1800,
                      3600) -- Yaw
    return nil, pos
end
    ...
    if (CRSF_PAUSE_TELEMETRY == true or rfsuite.app.triggers.mspBusy == true) then
        ...
        if command and data then
            ...
            if command == CRSF_FRAME_CUSTOM_TELEM then
                ...
                if delta > 1 then
                    elrs.telemetryFrameSkip = elrs.telemetryFrameSkip + 1
                end
                ...
                while ptr < #data do
                    ...
                    if sensor then
                        ...
                        if val then
                            setTelemetryValue(sid, 0, 0, val, sensor.unit,
                                              sensor.prec, sensor.name,
                ...
                setTelemetryValue(0xEE01, 0, 0, elrs.telemetryFrameCount,
                                  UNIT_RAW, 0, "Frame Count", 0, 2147483647) -- *Cnt
                setTelemetryValue(0xEE02, 0, 0, elrs.telemetryFrameSkip,
                                  UNIT_RAW, 0, "Frame Skip", 0, 2147483647) -- *Skp
...
function elrs.wakeup()
    ...
    if rfsuite.bg.telemetry.active() and rfsuite.rssiSensor then
        while elrs.crossfirePop() do
            if (CRSF_PAUSE_TELEMETRY == true or rfsuite.app.triggers.mspBusy ==
                true) then break end
        end
    end
end
