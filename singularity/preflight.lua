...
local function preflightWakeup(box, telemetry)
    ...
    if available == 0 then
        c.launch = "AWAITING LINK"
        c.launchColor = C.muted
        c.launchSub = "CONNECT TELEMETRY"
...
local function preflightPaint(x, y, w, h, box, c)
    ...
    drawTextAligned(x + 14, y + 8, w * 0.55, "SINGULARITY // PRE-LAUNCH", "FONT_STD", C.violet, "left")
    drawTextAligned(x + w - 260, y + 8, 246, c.launch or "AWAITING LINK", "FONT_STD", c.launchColor or C.muted, "right")
    ...
    local leftX = x + 14
    local rightX = x + w - nodeW - 14
    local topY = y + 54
    local lowY = y + h - nodeH - 14
    ...
    drawOrbitalMarker(cx, cy, coreR * 1.35, coreR * 1.02, 145, linkColor, 7)
    drawOrbitalMarker(cx, cy, coreR * 1.35, coreR * 1.02, 35, C.violet, 7)
end

local boxes_cache
local function boxes()
    if not boxes_cache then
        boxes_cache = {{col=1,row=1,colspan=12,rowspan=12,type="func",subtype="func",wakeup=preflightWakeup,paint=preflightPaint,bgcolor="transparent"}}
    end
    return boxes_cache
end
