...
local supportedProtocols = {
  sport = {
    ...
    mspQueueTimeout    = 6.0,         -- Maximum transaction time; long fragmented replies may still make progress
    mspRxInactivityTimeout = 0.90,   -- Retry only after no valid reply fragment arrives for this long
    mspQueueInterMessageDelay = 0.05, -- Conservative pacing for S.Port/F.Port
    mspQueueDrainAfterReplyMs = 0.05, -- Late-reply drain window, used only after retries
    mspQueueDrainMaxPolls = 5,
  ...
  crsf = {
    ...
    mspQueueTimeout    = 3.0,         -- Maximum transaction time (CRSF is low-latency)
    mspRxInactivityTimeout = 0.45,   -- CRSF fragments should arrive with much shorter gaps
    mspQueueInterMessageDelay = 0.01, -- CRSF can advance to the next command quickly
    mspQueueDrainAfterReplyMs = 0.03, -- Late-reply drain window, used only after retries
    mspQueueDrainMaxPolls = 5,
