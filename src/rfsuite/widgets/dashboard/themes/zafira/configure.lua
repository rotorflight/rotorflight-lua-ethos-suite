...
local function configure()
    ...
    plume:open(true)
    addField(plume:addLine("Maximum headspeed"), 100, 20000,
        function() return config.rpm_max end,
        function(v) config.rpm_max = clamp(tonumber(v) or DEFAULTS.rpm_max, 100, 20000) end,
        10, "rpm")

    local jewel = form.addExpansionPanel("Jewel power limits")
    jewel:open(false)
    addField(jewel:addLine("BEC critical"), 20, 150,
        function() return floor(config.bec_min * 10 + 0.5) end,
        function(v) config.bec_min = clamp((tonumber(v) or 20) / 10, 2.0, config.bec_warn - 0.1) end,
        nil, "V", 1)
    addField(jewel:addLine("BEC caution"), 20, 150,
        function() return floor(config.bec_warn * 10 + 0.5) end,
        function(v) config.bec_warn = clamp((tonumber(v) or 70) / 10, config.bec_min + 0.1, 15.0) end,
        nil, "V", 1)
    addField(jewel:addLine("Current caution"), 1, 500,
        function() return config.current_warn end,
        function(v) config.current_warn = clamp(tonumber(v) or DEFAULTS.current_warn, 1, 500) end,
        1, "A")
    addField(jewel:addLine("Power caution"), 100, 15000,
        function() return config.watts_warn end,
        function(v) config.watts_warn = clamp(tonumber(v) or DEFAULTS.watts_warn, 100, 15000) end,
        50, "W")
