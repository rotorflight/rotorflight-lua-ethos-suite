...
local function drawPlumeGauge(cx, cy, radius, rpm, maximum, color)
    ...
    for i = 0, feathers - 1 do
        local angle = 150 + 240 * i / (feathers - 1)
        local a = rad(angle)
        local inner = radius * 0.58
        local outer = radius * (0.88 + 0.08 * sin(pi * i / (feathers - 1)))
        local x1, y1 = cx + cos(a) * inner, cy + sin(a) * inner
        local x2, y2 = cx + cos(a) * outer, cy + sin(a) * outer
...
local function inflightPaint(x, y, w, h, box, c)
    ...
    drawEmberColumn(leftX, bodyY, sideW, halfH, c.esc, getThemeValue("esc_max"), escColor)
    ...
    local currentColor = c.current and c.current >= getThemeValue("current_warn") and C.red or C.fuchsia
