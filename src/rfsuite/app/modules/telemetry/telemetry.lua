...
local function alertIfTooManySensors()
    ...
    form.openDialog({width = nil, title = "Telemetry", message = "No more than 40 sensors can be enabled at once", buttons = buttons, wakeup = function() end, paint = function() end, options = TEXT_LEFT})
end
...
local function openPage(opts)
    ...
    if rfsuite.utils.apiVersionCompare("<", {12, 0, 8}) then
        ...
        rfsuite.app.formLines[#rfsuite.app.formLines + 1] = form.addLine("RF2.2 or later required")
...
local function wakeup()
    ...
    if triggerSave == true then
        rfsuite.app.ui.progressDisplaySave("Save settings")
...
local function onToolMenu(self)
    ...
    form.openDialog({width = nil, title = "Telemetry", message = "Apply default sensors?", buttons = buttons, wakeup = function() end, paint = function() end, options = TEXT_LEFT})
end
